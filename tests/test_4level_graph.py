...
def test_4level_decision_graph():
    ...
    print(f"Intermediate values - D: {expected_d}, E: {expected_e}, F: {expected_f}, G: {expected_g}")
...
def test_4level_graph_with_different_inputs():
    ...
    def multiply_func(x, y): return x * y
    def subtract_func(x, y): return x - y
    def divide_func(x, y): return x / y if y != 0 else 0
    def greater_than_func(x, y): return x > y
    def sum_func(x, y): return x + y
    ...
    nodes = [
        ...
        Node("D", False, "Sum D", "numeric", False, dependencies=["A", "B"], function=add_func),
        Node("E", False, "Product E", "numeric", False, dependencies=["B", "C"], function=multiply_func),
        Node("F", False, "Difference F", "numeric", False, dependencies=["D", "E"], function=subtract_func),
        Node("G", False, "Ratio G", "numeric", False, dependencies=["D", "E"], function=divide_func),
        Node("H", False, "Decision H", "boolean", True, dependencies=["F", "G"], function=greater_than_func),
        Node("I", False, "Decision I", "numeric", True, dependencies=["F", "G"], function=sum_func)
    ]
    ...
    print(f"Input A: {input_values['A']}, B: {input_values['B']}, C: {input_values['C']}")
...
def test_memoization_efficiency():
    ...
    nodes = [
        ...
        Node("D", False, "Sum D", "numeric", False, dependencies=["A", "B"], function=add_func),
        Node("F", False, "Double F", "numeric", False, dependencies=["D"], function=double_func),
        Node("G", False, "Triple G", "numeric", False, dependencies=["D"], function=triple_func),
        Node("H", False, "Decision H", "numeric", True, dependencies=["F"], function=lambda x: x),
        Node("I", False, "Decision I", "numeric", True, dependencies=["G"], function=lambda x: x)
    ]
...
def test_node_value_updates():
    ...
    nodes = [
        Node("A", True, "Input A", "numeric", False, value=5.0),
        Node("B", False, "Double B", "numeric", False, dependencies=["A"], function=double_func),
        Node("C", False, "Double C", "numeric", True, dependencies=["B"], function=double_func)
    ]
    ...
    print(f"Individual queries - A: {graph.get_node_value('A')}, B: {graph.get_node_value('B')}, C: {graph.get_node_value('C')}")
...
def test_cache_vs_results_separation():
    ...
    nodes = [
        Node("A", True, "Input A", "numeric", False, value=5.0),
        Node("B", False, "Sum B", "numeric", False, dependencies=["A"], function=double_func),
        Node("C", False, "Final C", "numeric", True, dependencies=["B"], function=double_func)
    ]
