...
def create_decision_sensitivity_content():
    ...
    return html.Div([
        ...
        html.P("Sensitivity analysis for decision parameters and their impact on outcomes."),
        ...
        dbc.Row([
            dbc.Col([
                ...
                dbc.Input(type="number", value=10, min=0, max=50, step=1)
            ], width=4),
            dbc.Col([
                html.Label("Climate Change Factor:"),
                dbc.Input(type="number", value=1.2, min=0.5, max=2.0, step=0.1)
            ], width=4),
            dbc.Col([
                html.Label("Economic Sensitivity:"),
                dbc.Input(type="number", value=15, min=5, max=30, step=1)
            ], width=4)
        ], className="mb-3"),
        html.H5("Sensitivity Results:"),
        html.P("The sensitivity analysis shows how changes in key parameters affect the overall project viability and recommendations.")
    ])
